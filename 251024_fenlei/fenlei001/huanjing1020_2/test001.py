...
REGULATION_STAGES = {
    '命令控制阶段': ['命令控制', '政府主导', '强制监管'],
    '工具扩展阶段': ['污染税', '排污权交易', '市场激励'],
    '多元协同阶段': ['多元协同', '公众监督', '非政府组织']
}

POLICY_TOOLS = {
    '命令控制型': ['强制', '必须', '禁止', '处罚'],
    '市场激励型': ['补贴', '奖励', '税收优惠', '碳交易'],
    '自愿参与型': ['自愿', '倡议', '承诺', '认证'],
    '多元协同型': ['公众监督', 'NGO', '社会组织']
}
...
def is_relevant(text):
    """判断是否与核心主题相关"""
    text_lower = text.lower()
    core_kw = [kw for sublist in KEYWORDS.values() for kw in sublist]
    return any(kw.lower() in text_lower for kw in core_kw)
...
def process_document(file_path):
    ...
    stages = classify_content(text, REGULATION_STAGES)
    ...
    return {
        '文件名': file_name,
        '文件路径': file_path,
        '年份': year if year else '未确定年份',
        '是否相关': '是' if relevant else '否',
        '层级': ';'.join(levels),
        '区域': ';'.join(regions),
        '文件类型': ';'.join(doc_types),
        '规制阶段': ';'.join(stages),
        '政策工具': ';'.join(policy_tools),
        '关键词': ';'.join(keywords)
    }
...
def save_classification_report(results, report_path):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        for i, result in enumerate(results, 1):
            f.write(f"文档 {i}: {result['文件名']}\n")
            f.write(f"路径: {result['文件路径']}\n")
            f.write(f"年份: {result['年份']}\n")
            f.write(f"是否相关: {result['是否相关']}\n")
            f.write(f"层级: {result['层级']}\n")
            f.write(f"地区: {result['区域']}\n")
            f.write(f"文档类型: {result['文件类型']}\n")
            f.write(f"规制阶段: {result['规制阶段']}\n")
            f.write(f"政策工具: {result['政策工具']}\n")
            f.write(f"关键词: {result['关键词']}\n")
            f.write("-" * 80 + "\n")
...
def generate_detailed_report(df, output_dir):
    ...
    summary.append(f"相关文件数：{relevant_count} 份（占比 {round(relevant_count/total*100, 2)}%）")
    summary.append(f"不相关文件数：{total - relevant_count} 份\n")
    ...
    for region, cnt in sorted(region_counts.items(), key=lambda x: -x[1]):
        summary.append(f"- {region}：{cnt} 份（占比 {round(cnt/relevant_count*100, 2)}%）")
    ...
    for t, cnt in sorted(type_counts.items(), key=lambda x: -x[1]):
        summary.append(f"- {t}：{cnt} 份（占比 {round(cnt/relevant_count*100, 2)}%）")
    ...
    for stage, cnt in sorted(stage_counts.items(), key=lambda x: -x[1]):
        summary.append(f"- {stage}：{cnt} 份（占比 {round(cnt/relevant_count*100, 2)}%）")

    # 政策工具分布（修复变量名错误）
    tool_counts = defaultdict(int)
    for tools in relevant['政策工具']:
        for tool in tools.split(';'):
            tool_counts[tool] += 1
    summary.append("\n----- 政策工具分布 -----")
    for tool, cnt in sorted(tool_counts.items(), key=lambda x: -x[1]):
        # 修复此处的变量名错误，将tool_cnt改为cnt
        summary.append(f"- {tool}：{cnt} 份（占相关文件 {round(cnt/relevant_count*100, 2)}%）")
...
def organize_files(results, output_dir):
    ...
    for result in results:
        ...
        # 创建分类路径
        year_dir = str(result['年份']) if result['年份'] != '未确定年份' else '年份未明确'
        level_dir = result['层级'].split(';')[0]
        region_dir = result['区域'].split(';')[0]
        type_dir = result['文件类型'].split(';')[0]
        ...
        # 复制文件（处理重名）
        src_path = result['文件路径']
        dest_path = os.path.join(target_path, result['文件名'])
        counter = 1

        while os.path.exists(dest_path):
            name, ext = os.path.splitext(result['文件名'])
            dest_path = os.path.join(target_path, f"{name}_{counter}{ext}")
            counter += 1

        shutil.copy2(src_path, dest_path)
        print(f"已复制: {result['文件名']} -> {target_path}")
...
def main():
    ...
    # 保存详细分类报告
    detail_report_path = os.path.join(input_dir, "详细分类报告.txt")
    save_classification_report(results, detail_report_path)
    print(f"详细分类报告已保存至: {detail_report_path}")

    # 生成总结报告
    generate_detailed_report(df, input_dir)

    # 组织文件
    organize = input("\n是否要按分类结果整理相关文件? (y/n): ").strip().lower()
    if organize == 'y':
