...
import PyPDF2
import subprocess
import textract  # 用于多格式文件读取（优先依赖）
...
# 修复：移除Windows不存在的"Heiti TC"字体，保留常用中文字体（解决字体丢失警告）
matplotlib.use('Agg')  # 非交互式环境（避免绘图报错）
plt.rcParams["font.family"] = ["SimHei", "Microsoft YaHei", "Arial Unicode MS"]  # 仅保留Windows兼容字体
...
CORE_KEYWORDS = [
    ...
    '环境规划', '环境标准', '排污许可', '环境影响评价', '生态环境治理',
    '绿色发展基金', '环保督察', '环境监管', '节能减排'
]
START_YEAR = 2015
END_YEAR = 2025
...
# -------------------------- 文件读取修复（解决.doc文件读取失败问题） --------------------------
def read_file_content(file_path):
    """读取txt/docx/pdf/doc内容，优先用textract，减少antiword依赖"""
    ...
    try:
        if ext == '.txt':
            ...
            # 修复：优先用textract读取.doc，失败则提示用户转换格式（而非依赖antiword）
            doc_content = read_doc(file_path)
            if not doc_content.strip():
                logger.warning(f".doc文件内容为空，建议手动转换为.docx后重试：{file_path}")
            return doc_content
        else:
...
def read_txt(file_path):
    ...
    for encoding in encodings:
        try:
            ...
            continue
    logger.error(f"无法解析txt文件编码：{file_path}（建议用记事本打开后另存为UTF-8格式）")
    return ""


def read_docx(file_path):
    """读取docx文件（保持原逻辑，增加textract备选）"""
    try:
        ...
        if not content.strip():
            logger.warning(f"docx正文为空，尝试用textract重试：{file_path}")
            content = textract.process(file_path).decode('utf-8', errors='ignore')
        return content
    except Exception as e:
        logger.error(f"docx读取失败 {file_path}：{str(e)}，尝试textract备选方案")
        try:
            return textract.process(file_path).decode('utf-8', errors='ignore')
        except Exception as te:
            logger.error(f"textract读取docx也失败：{str(te)}")
            return ""


def read_pdf(file_path):
    """读取pdf文件（保持原逻辑，增加加密文件提示）"""
    text = ""
    try:
        with open(file_path, 'rb') as f:
            ...
            if reader.is_encrypted:
                try:
                    ...
                    logger.warning(f"加密PDF无法读取（需手动解密）：{file_path}")
                    return ""
            for page in reader.pages:
                ...
                if page_text:
                    text += page_text + " "
        if not text.strip():
            logger.warning(f"PDF文本提取为空，尝试textract重试：{file_path}")
            text = textract.process(file_path).decode('utf-8', errors='ignore')
        return text
    except Exception as e:
        logger.error(f"PDF读取失败 {file_path}：{str(e)}，尝试textract备选方案")
        try:
            return textract.process(file_path).decode('utf-8', errors='ignore')
        except Exception as te:
            logger.error(f"textract读取PDF也失败：{str(te)}")
            return ""


def read_doc(file_path):
    """修复：弱化antiword依赖，优先textract，失败则提示用户转换格式（而非报错）"""
    try:
        # 1. 优先用textract读取.doc（支持更多Windows环境）
        text = textract.process(file_path).decode('utf-8', errors='ignore')
        if text.strip():
            return text
        # 2. textract失败时，提示用户安装依赖或转换格式（而非强制调用antiword）
        logger.warning(f"textract读取.doc失败，建议：1. 安装libreoffice；2. 手动将.doc转.docx；3. 安装antiword")
        logger.info(f"依赖安装命令参考：pip install textract[all] 或 官网下载libreoffice（https://www.libreoffice.org/）")

        # 3. 保留antiword尝试（但仅作为最后选项，且不报错）
        try:
            result = subprocess.run(
                ...
                encoding='utf-8',
                timeout=30,
                shell=True  # 修复Windows下命令调用问题
            )
            if result.returncode == 0 and result.stdout.strip():
                return result.stdout
            else:
                logger.warning(f"antiword执行失败（返回码：{result.returncode}），请检查是否安装antiword并配置环境变量")
                return ""
        except FileNotFoundError:
            logger.warning(f"未找到antiword命令（Windows需手动安装并添加到环境变量）")
            return ""
        except Exception as ae:
            logger.error(f"antiword读取.doc失败：{str(ae)}")
            return ""
    except Exception as e:
        logger.error(f".doc文件读取总失败 {file_path}：{str(e)}")
        return ""
...
def manual_sampling_verification(df, output_dir, sample_ratio=0.15):
    ...
    try:
        ...
        logger.warning(f"分层抽样失败：{str(e)}，使用简单随机抽样")
    ...
    try:
        ...
        logger.info(f"待验证样本已保存至：{verification_file}")
    except Exception as e:
        logger.error(f"保存待验证样本失败：{str(e)}")
...
def analyze_literature_gaps(df, cross_analyses, output_dir):
    ...
    if not year_dist.empty:
        ...
        for i in range(1, len(years)):
            prev_year = years[i - 1]
            curr_year = years[i]
            if year_dist[curr_year] < year_dist[prev_year] * 0.3:
...
def main():
    ...
    try:
        ...
        if len(sys.argv) > 2:
            ...
            # 默认目录（使用绝对路径，避免相对路径错误）
            input_directory = r"D:\Desktop\效力位阶"
            output_directory = r"D:\Desktop\新建文件夹"
            logger.info(f"未指定命令行参数，使用默认目录：输入={input_directory}，输出={output_directory}")

        # 确保输入目录存在
        if not os.path.exists(input_directory):
            logger.error(f"输入目录不存在：{input_directory}，请检查路径")
            sys.exit(1)
        os.makedirs(output_directory, exist_ok=True)

        # 执行分析（抽取15%样本验证）
        process_files(input_directory, output_directory, sample_ratio=0.15)
    except Exception as e:
