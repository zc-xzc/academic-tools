...
# 设置中文字体和非交互式环境
matplotlib.use('Agg')
plt.rcParams["font.family"] = ["SimHei", "Microsoft YaHei"]  # 使用系统中可用的中文字体
...
def read_file_content(file_path):
    ...
    try:
        if ext == '.txt':
            ...
            return read_doc(file_path)  # 添加对 .doc 文件的支持
        else:
...
def read_doc(file_path):
    ...
    try:
        ...
        print(f"无法读取 .doc 文件 {file_path}：{str(e)}")
        return ""
...
def manual_sampling_verification(df, output_dir, sample_ratio=0.1):
    ...
    if os.path.exists(verification_file):
        try:
            ...
            # 检查是否有完成的验证
            has_verification = not verified_df['主要政府层级'].isna().all()

            if has_verification:
                # 过滤掉未验证的行
                valid_rows = verified_df.dropna(subset=['主要政府层级'])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        # 默认目录
        input_directory = "D:\Desktop\效力位阶"  # 输入文件目录
        output_directory = "D:\Desktop\新建文件夹"  # 分析结果输出目录
