...
AUTO_NAVIGATION_DELAY = random.uniform(2.0, 3.0)  # 自动翻页延迟
...
HUMAN_RANDOM_SCROLL = (0, 0)  # 禁用随机滚动
...
auto_mode = False  # 全自动模式开关
download_count_mode = "auto"  # 默认为自动确定下载数量
region_memory = []  # 用于记忆下载按钮区域
batch_size = 4  # 分批下载大小
...
def detect_new_file(expecting=None):
    ...
    if new_files:
        ...
        if expecting:
            ...
            for exp in expecting:
                if len(file_name) >= 5 and len(exp) >= 5 and file_name[:5] == exp[:5]:
                    match = True
                    break
            if not match:
                print(f"⚠️ 下载文件不匹配预期: {file_name} 预期: {expecting}")
...
def check_duplicate_files():
    """检查下载目录中的重复文件（基于大小和名称前缀）"""
    file_sizes = defaultdict(list)
    duplicates = []

    for f, size in initial_files:
        file_sizes[size].append(f)
    ...
    if duplicates:
        print(f"⚠️ 发现重复文件: {', '.join(duplicates)}")
        return True
    return False
...
def navigate_to_page(direction):
    ...
    if direction == "next":
        ...
        try:
            ...
            # 方法2：点击下一页按钮（如果存在）
            next_btn_pos = pyautogui.locateOnScreen('next_page.png', confidence=0.7)
            if next_btn_pos:
                center = pyautogui.center(next_btn_pos)
...
def select_region_on_screenshot(screenshot_path, is_first_page=True):
    ...
    # 如果有记忆的区域，尝试自动框选
    if not is_first_page and len(region_memory) > 2:  # 至少需要3次框选才能形成记忆
        if auto_mode:
            # 自动模式下使用记忆区域
            avg_region = calculate_average_region()
            x1, y1, x2, y2 = avg_region
            ref_point = [(x1, y1), (x2, y2)]
            box_selected = True
            cv2.rectangle(img_copy, ref_point[0], ref_point[1], (0, 255, 0), 4)
            cv2.putText(img_copy, f"✅ 使用记忆区域！按ESC关闭", (30, 250), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            cv2.imshow(WINDOW_TITLE, img_copy)
            print(f"✅ 自动使用记忆区域：({x1},{y1})→({x2},{y2})")
            time.sleep(2)
    ...
    # 等待操作（延长超时到90秒）
    timeout = 90 if not auto_mode else 10  # 自动模式下超时时间缩短
    start_time = time.time()
    while True:
        key = cv2.waitKey(1) & 0xFF
        elapsed = time.time() - start_time

        if elapsed > 60 and not box_selected and not auto_mode:
    ...
    if len(ref_point) == 2 and box_selected:
        ...
        # 记录区域用于记忆
        region_memory.append((x1, y1, x2, y2))
        # 只保留最近5次的框选记录
        if len(region_memory) > 5:
            region_memory.pop(0)

        # 翻页后增加区域确认提示
        if not is_first_page and not auto_mode:
...
def calculate_average_region():
    """计算记忆区域的平均值，用于自动框选"""
    if not region_memory:
        return None

    x1_sum, y1_sum, x2_sum, y2_sum = 0, 0, 0, 0
    count = len(region_memory)
    ...
    return (x1_sum // count, y1_sum // count, x2_sum // count, y2_sum // count)
...
def auto_detect_target_count(download_img):
    ...
    # 根据模式选择是否手动输入
    if download_count_mode == "auto" or auto_mode:
        target_count = detected_count
        print(f"✅ 自动使用识别到的 {detected_count} 个下载按钮")
    else:
        # 提示用户确认，支持手动修正
...
def get_document_title(pos):
    """获取文献标题（简化版，实际可能需要更复杂的OCR）"""
    # 假设标题在下载按钮左侧
    x, y = pos
    title_x = x - 300  # 向左偏移300像素
    title_y = y

    # 截图标题区域
    try:
        bbox = (title_x, title_y - 20, x - 20, title_y + 20)
        title_img = ImageGrab.grab(bbox=bbox)
        # 这里简化处理，实际应用中应该使用OCR识别文字
        # 为了演示，返回一个假的标题前缀
        return f"doc_{title_x}_{title_y}"
    except:
        return f"unknown_doc_{x}_{y}"
...
def click_download_button(btn_idx, pos):
    ...
    # 获取文献标题用于验证
    doc_title = get_document_title(pos)
    print(f"\n📥 下载第{btn_idx + 1}/{target_count}个（坐标：{int(final_x)},{int(final_y)}，标题前缀：{doc_title[:5]}）")
    ...
    if button_color == "yellow":
        if auto_mode:
            print("⚠️  自动模式下检测到黄色按钮，需要人工干预！")
            print("请手动完成验证，完成后按回车继续...")
            input("👉 验证完成后按回车：")
        else:
    ...
    while time.time() - start_time < DOWNLOAD_TIMEOUT:
        if detect_new_file([doc_title]):  # 传递预期标题用于验证
            # 下载完成后随机间隔
...
def on_key_press(key):
    global is_running, is_paused, auto_mode
    try:
        ...
        elif key == keyboard.Key.enter and not is_running:
            # 在截图后按回车进入全自动模式
            auto_mode = True
            is_running = True
            print("\n▶️ 进入全自动模式，开始自动下载...")
...
def process_single_page(download_img):
    ...
    # 分批下载
    total_downloaded = 0
    batches = [buttons[i:i + batch_size] for i in range(0, len(buttons), batch_size)]

    for batch_idx, batch in enumerate(batches):
        if not is_running:
            break

        print(f"\n📦 开始第{batch_idx + 1}批下载（共{len(batches)}批）")
        for idx, btn_pos in enumerate(batch):
            while is_paused:
                time.sleep(0.5)
                if not is_running:
                    return False
            ...
            if (total_downloaded + 1) % HUMAN_PAUSE_TRIGGER == 0 and total_downloaded != 0:
                human_like_pause()

            if click_download_button(total_downloaded, btn_pos):
    ...
    if check_duplicate_files():
        print("⚠️ 发现重复文件，可能存在下载错误")
        if not auto_mode:
            retry = input("👉 是否重新下载当前页？（y/n）：").strip().lower()
            if retry == "y":
                return process_single_page(download_img)

    # 检查是否下载了预期数量的文件（允许20%误差）
    if current_page_downloaded >= target_count * 0.8:
        print(f"\n🎉 第{current_page}页下载完成（{current_page_downloaded}/{target_count}个）")
        return True
    else:
        print(f"\n⚠️  第{current_page}页未完成（{current_page_downloaded}/{target_count}个）")
        # 自动模式下尝试重新下载
        if auto_mode and retry_count < max_retry:
            retry_count += 1
            print(f"🔄 自动重试第{retry_count}次下载当前页...")
            return process_single_page(download_img)
        return False
...
def main():
    # 声明使用的全局变量
    global is_running, is_paused, downloaded_count, current_page, target_count, auto_mode, download_count_mode

    print("=" * 75)
    print("📌 知网批量下载终极优化版（全自动+智能识别）")
    print("✅ 核心特性：")
    print("  1. 支持全自动模式：截图后按回车自动完成所有操作")
    print("  2. 智能记忆下载区域，减少重复框选")
    print("  3. 分批下载与文件验证，确保下载完整性")
    print("  4. 支持手动/自动选择下载数量")
    print("  5. 优化翻页功能，确保自动翻页可靠")
    print("✅ 快捷键：ESC=停止 | 空格=暂停/继续 | 截图后回车=进入全自动模式")
    print("=" * 75)

    # 选择模式
    while True:
        mode_choice = input("请选择运行模式（1=全自动模式 2=手动模式）：").strip()
        if mode_choice == "1":
            auto_mode = True
            download_count_mode = "auto"
            print("✅ 已选择全自动模式")
            break
        elif mode_choice == "2":
            auto_mode = False
            while True:
                count_choice = input("请选择下载数量模式（1=自动识别 2=手动输入）：").strip()
                if count_choice == "1":
                    download_count_mode = "auto"
                    break
                elif count_choice == "2":
                    download_count_mode = "manual"
                    break
                else:
                    print("❌ 输入无效，请输入1或2")
            print("✅ 已选择手动模式")
    ...
    # 临时将is_running设为False，等待用户截图后按回车进入全自动模式
    temp_running_state = is_running
    if auto_mode:
        is_running = False
        print("⚠️  请完成截图和框选，然后按回车键进入全自动模式")
    ...
    try:
        while is_running:
            ...
            # 下载完成后翻页（自动模式）或询问（手动模式）
            nav_choice = confirm_navigation()
            if nav_choice == "n":
                # 不翻页，询问是否继续下载当前页或停止
                if auto_mode:
                    # 自动模式下，连续两次下载同一页仍未完成则停止
                    print(f"⚠️  自动模式下检测到无法完成第{current_page}页下载，停止运行")
                    is_running = False
                    break
                else:
