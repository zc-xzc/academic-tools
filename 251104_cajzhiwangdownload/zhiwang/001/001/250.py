...
import pytesseract  # 用于OCR识别文献编号
from fuzzywuzzy import fuzz  # 用于模糊匹配判断编号相似性

# ==================== 新增配置项 ====================
# OCR识别配置
OCR_REGION_OFFSET = (-60, -10, -10, 10)  # 文献编号区域相对下载按钮的偏移 (左,上,右,下)
DUPLICATE_THRESHOLD = 80  # 编号相似度阈值，超过此值判定为重复
TESSERACT_PATH = r"C:\Program Files\Tesseract-OCR\tesseract.exe"  # tesseract安装路径

# 时间设置模式（自动/手动）
TIME_MODE = "auto"  # 默认自动模式
...
downloaded_ids = set()  # 已下载文献编号集合
auto_mode = False  # 全自动模式开关（下载完成自动翻页）


# ==================== 新增：OCR识别函数 ====================
def init_ocr():
    """初始化OCR引擎"""
    global TESSERACT_PATH  # 移到函数开头声明
    if not os.path.exists(TESSERACT_PATH):
        print("\n❌ 未找到Tesseract-OCR，请先安装并配置路径")
        print("   下载地址：https://github.com/UB-Mannheim/tesseract/wiki")
        print(f"   当前配置路径：{TESSERACT_PATH}")
        new_path = input("   请输入Tesseract-OCR安装路径（tesseract.exe）：").strip()
        if os.path.exists(new_path):
            TESSERACT_PATH = new_path
            print("✅ OCR路径已更新")
        else:
            print("❌ 无效路径，将无法进行重复检测")
            return False
...
def get_document_id(button_pos):
    ...
    x, y = button_pos
    # 计算编号区域（相对按钮左侧）
    left = x + OCR_REGION_OFFSET[0]
    top = y + OCR_REGION_OFFSET[1]
    right = x + OCR_REGION_OFFSET[2]
    bottom = y + OCR_REGION_OFFSET[3]
...
def setup_time_parameters():
    ...
    while True:
        ...
        if choice == "1":
            ...
            while True:
                try:
                    min_move = float(input("鼠标移动最小时间："))
                    max_move = float(input("鼠标移动最大时间："))
                    if min_move >= 0 and max_move > min_move:
                        HUMAN_MOVE_DURATION = (min_move, max_move)
                        break
                    print("❌ 请确保最大时间大于最小时间")
                except ValueError:
                    print("❌ 请输入有效数字")
            ...
            while True:
                try:
                    min_stay = float(input("点击前最小停留时间："))
                    max_stay = float(input("点击前最大停留时间："))
                    if min_stay >= 0 and max_stay > min_stay:
                        HUMAN_STAY_DURATION = (min_stay, max_stay)
                        break
                    print("❌ 请确保最大时间大于最小时间")
                except ValueError:
                    print("❌ 请输入有效数字")
            ...
            while True:
                try:
                    min_dl = float(input("下载间隔最小时间："))
                    max_dl = float(input("下载间隔最大时间："))
                    if min_dl >= 0 and max_dl > min_dl:
                        HUMAN_DOWNLOAD_INTERVAL = (min_dl, max_dl)
                        break
                    print("❌ 请确保最大时间大于最小时间")
                except ValueError:
                    print("❌ 请输入有效数字")
            ...
            while True:
                try:
                    min_pause = float(input("随机休息最小时间："))
                    max_pause = float(input("随机休息最大时间："))
                    if min_pause >= 0 and max_pause > min_pause:
                        HUMAN_PAUSE_INTERVAL = (min_pause, max_pause)
                        break
                    print("❌ 请确保最大时间大于最小时间")
                except ValueError:
                    print("❌ 请输入有效数字")
...
def select_run_mode():
    ...
    while True:
        choice = input("请选择运行模式（1=全自动模式/2=手动翻页模式）：").strip()
        if choice == "1":
            auto_mode = True
            print("✅ 已选择全自动模式")
            print("📝 模式说明：")
    ...
    if direction == "next":
        ...
        try:
            ...
            if next_btn_pos:
                ...
                human_like_move_to(center.x, center.y)
                pyautogui.click()
                success = True
        except:
            pass
...
def confirm_navigation():
    """询问用户是否继续翻页（全自动模式下自动翻页）"""
    if auto_mode:
        print("\n▶️ 全自动模式：当前页下载完成，自动翻页...")
        return "y"
...
def select_region_on_screenshot(screenshot_path, is_first_page=True):
    ...
    if is_first_page:
        ...
        if auto_mode:
            cv2.putText(img_copy, "⚠️  全自动模式：框选后自动开始下载", (30, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (0, 255, 255), 2)
        else:
            cv2.putText(img_copy, "⚠️  若区域不对请重新框选", (30, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255),
                        2)
...
def auto_detect_target_count(download_img):
    ...
    # 全自动模式直接使用识别数量，手动模式允许修正
    if auto_mode:
        target_count = detected_count
        print(f"✅ 全自动模式：使用识别到的 {detected_count} 个下载按钮")
    else:
        # 提示用户确认，支持手动修正
...
def click_download_button(btn_idx, pos):
    ...
    if doc_id and is_duplicate(doc_id):
...
def main():
    ...
    print("=" * 75)
    print("📌 知网批量下载终极优化版（全自动+防重复+智能翻页）")
    print("✅ 核心特性：")
    print("  1. 支持全自动/手动翻页两种模式")
    print("  2. 文献编号OCR识别，精准防重复下载")
    print("  3. 自动/手动时间设置，灵活控制下载节奏")
    print("  4. 多方式翻页优化，解决翻页失败问题")
    print("  5. 蓝色按钮全自动，黄色按钮人工验证提示")
    ...
    try:
        while is_running:
            ...
            if nav_choice == "n":
                # 不翻页，询问是否继续下载当前页或停止
                if auto_mode:
                    print(f"⚠️  全自动模式下检测到无法继续翻页，停止运行")
                    is_running = False
                    break
                else:
