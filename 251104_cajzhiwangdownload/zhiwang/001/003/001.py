...
# 按钮1偏移策略：0、±2、±4、±6（双边推进）
PAGE1_BUTTON1_OFFSETS = [0, 2, -2, 4, -4, 6, -6]
PAGE1_BUTTON2_OFFSET_STEP = 4  # 按钮2及以后偏移步长（改为4像素）

# 页面二专属配置
PAGE2_BUTTON1_OFFSETS = [0, 2, -2, 4, -4, 6, -6]  # 统一为双边推进策略
PAGE2_STEP1_OFFSET = 78  # 第一步基础偏移（改为78像素）
PAGE2_STEP2_OFFSET = 6  # 第二步偏移步长
PAGE2_STEP2_COUNT = 3  # 第二步尝试次数
PAGE2_STEP3_OFFSET = 4  # 第三步偏移步长（改为4像素）
...
def download_page1(buttons):
    """页面一下载逻辑（按钮2及以后偏移策略调整为0、4、-4、8、-8...）"""
    global downloaded_total, page1_buttons, last_action_time
        ...
        def offset_strategy(i):
            if idx == 0:
                ...
                if i == 0:
                    ...
                    step = ((i + 1) // 2) * PAGE1_BUTTON2_OFFSET_STEP
...
# ==================== 页面二下载逻辑 ====================
def download_page2():
    """页面二下载逻辑（STEP1_OFFSET=78，第三步偏移步长4）"""
    global downloaded_total, last_action_time
    ...
    for idx in range(button_count):
        ...
        if idx == 0:
            ...
            prev_btn = page1_buttons[idx - 1]
            prev_final_y = prev_btn[1] + prev_btn[2]
            target_y = prev_final_y + PAGE2_STEP1_OFFSET  # 基础偏移（改为78px）
        ...
        def offset_strategy(i):
            if idx == 0:
                ...
                if i == 0:
                    ...
                    # 第二步：6、-6、12、-12、18（假设STEP2_COUNT=3时共5次尝试）
                    return PAGE2_STEP2_OFFSET * i if i % 2 == 1 else -PAGE2_STEP2_OFFSET * (i // 2 + 1)
                else:
                    # 第三步：从22开始，按4px步长双边推进（22,-22,26,-26...）
                    step3_idx = i - PAGE2_STEP2_COUNT - 1  # 第三步起始索引（0开始）
                    base_step = 18 + PAGE2_STEP3_OFFSET  # 18是第二步最大偏移，加4得22
                    step = base_step + (step3_idx // 2) * PAGE2_STEP3_OFFSET * 2
                    return step if step3_idx % 2 == 0 else -step
...
def on_key_press(key):
    global is_running, is_paused, last_action_time
    try:
