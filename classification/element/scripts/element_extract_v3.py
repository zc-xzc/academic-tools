...
# 环境配置（修复字体问题，仅保留Windows系统常见中文字体）
matplotlib.use('Agg')
plt.rcParams["font.family"] = ["SimHei", "Microsoft YaHei"]  # 移除Arial Unicode MS
...
def generate_cross_analysis(df, output_dir):
    ...
    # 5. 各维度分布统计（修复百分比格式化错误）
    report.append("\n----- 5. 各维度分布统计 -----")

    # 政府层级分布（使用正确的百分比格式）
    level_dist = df['主要政府层级'].value_counts(normalize=True)
    report.append("政府层级分布：")
    report.append(level_dist.to_string(float_format='%.2f%%'))  # 修正为%.2f%%

    # 文件类型分布
    type_dist = df['主要文件类型'].value_counts(normalize=True)
    report.append("\n文件类型分布：")
    report.append(type_dist.to_string(float_format='%.2f%%'))  # 修正为%.2f%%

    # 环境要素分布
    env_dist = df['主要环境要素'].value_counts(normalize=True)
    report.append("\n环境要素分布：")
    report.append(env_dist.to_string(float_format='%.2f%%'))  # 修正为%.2f%%

    # 政策工具分布
    tool_dist = df['主要政策工具'].value_counts(normalize=True)
    report.append("\n政策工具分布：")
    report.append(tool_dist.to_string(float_format='%.2f%%'))  # 修正为%.2f%%

    # 区域分布
    region_dist = df['主要区域'].value_counts(normalize=True)
    report.append("\n区域分布：")
    report.append(region_dist.to_string(float_format='%.2f%%'))  # 修正为%.2f%%
