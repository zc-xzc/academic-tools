...
PAGE1_BUTTON2_OFFSET_STEP = 6  # 按钮2及以后偏移步长（延续双边推进逻辑）

# 页面二专属配置
PAGE2_BUTTON1_OFFSETS = [0, 2, -2, 4, -4, 6, -6]  # 统一为双边推进策略
PAGE2_STEP1_OFFSET = 65  # 第一步基础偏移
PAGE2_STEP2_OFFSET = 6   # 第二步偏移步长（改为6像素，保持一致性）
PAGE2_STEP2_COUNT = 3    # 第二步尝试次数
PAGE2_STEP3_OFFSET = 6   # 第三步偏移步长（改为6像素）
        ...
        def offset_strategy(i):
            if idx == 0:
                ...
                if i == 0:
                    ...
                    return PAGE2_STEP3_OFFSET * (step3_idx + 1) if step3_idx % 2 == 0 else -PAGE2_STEP3_OFFSET * (step3_idx + 1)
