...
def create_processed_dir(base_dir, mode):
    """根据模式创建 processed 文件夹"""
    if mode == 1:  # 子目录模式
...
def process_files(df, doc_dir, processed_dir, mode):
    """处理文件并根据模式复制或移动"""
...
def find_directories(root_dir, mode):
    """查找包含表格文件和文档文件的目录"""
    valid_dirs = []
        ...
        if any(f.endswith(('.xls', '.xlsx')) for f in filenames):
            for dirname in dirnames:
                if dirname.lower() in ['doc', 'txt']:
                    valid_dirs.append((dirpath, os.path.join(dirpath, dirname)))
    return valid_dirs
...
if __name__ == "__main__":
    ...
    valid_directories = find_directories(root_dir, mode)
    ...
    if not valid_directories:
        ...
        for table_dir, doc_dir in valid_directories:
            ...
            # 创建目标目录
            processed_dir = create_processed_dir(doc_dir if mode == 3 else table_dir, mode)

            # 处理文件
            process_files(df, doc_dir, processed_dir, mode)
